...
// ─── Investigation States & Transitions ────────────
...
export type InvestigationState = typeof INVESTIGATION_STATES[keyof typeof INVESTIGATION_STATES];

export const VALID_INVESTIGATION_TRANSITIONS: Record<InvestigationState, InvestigationState[]> = {
  [INVESTIGATION_STATES.PENDING]: [INVESTIGATION_STATES.COLLECTING_EVIDENCE, INVESTIGATION_STATES.FAILED],
  [INVESTIGATION_STATES.COLLECTING_EVIDENCE]: [INVESTIGATION_STATES.ANALYZING, INVESTIGATION_STATES.FAILED, INVESTIGATION_STATES.HUMAN_REVIEW],
  [INVESTIGATION_STATES.ANALYZING]: [INVESTIGATION_STATES.HYPOTHESIS_GENERATED, INVESTIGATION_STATES.FAILED, INVESTIGATION_STATES.HUMAN_REVIEW],
  [INVESTIGATION_STATES.HYPOTHESIS_GENERATED]: [INVESTIGATION_STATES.DIAGNOSED, INVESTIGATION_STATES.FAILED, INVESTIGATION_STATES.HUMAN_REVIEW],
  [INVESTIGATION_STATES.DIAGNOSED]: [INVESTIGATION_STATES.RECOMMENDATIONS_READY, INVESTIGATION_STATES.FAILED, INVESTIGATION_STATES.HUMAN_REVIEW],
  [INVESTIGATION_STATES.RECOMMENDATIONS_READY]: [INVESTIGATION_STATES.HUMAN_REVIEW],
  [INVESTIGATION_STATES.HUMAN_REVIEW]: [],
  [INVESTIGATION_STATES.FAILED]: [],
};

// ─── Evidence Types ────────────────────────────────
export const EVIDENCE_TYPES = {
  PAYMENT_METRIC: 'PAYMENT_METRIC',
  FAILURE_DISTRIBUTION: 'FAILURE_DISTRIBUTION',
  BANK_SIGNAL: 'BANK_SIGNAL',
  PAYMENT_METHOD_SIGNAL: 'PAYMENT_METHOD_SIGNAL',
...
export const CANDIDATE_HYPOTHESES = {
  BANK_DEGRADATION: 'BANK_DEGRADATION',
  PAYMENT_METHOD_DEGRADATION: 'PAYMENT_METHOD_DEGRADATION',
  BANK_PAYMENT_METHOD_DEGRADATION: 'BANK_PAYMENT_METHOD_DEGRADATION',
  GATEWAY_DEGRADATION: 'GATEWAY_DEGRADATION',
  REGIONAL_DEGRADATION: 'REGIONAL_DEGRADATION',
  DEVICE_SPECIFIC_DEGRADATION: 'DEVICE_SPECIFIC_DEGRADATION',
  TRAFFIC_SPIKE: 'TRAFFIC_SPIKE',
  MERCHANT_CONFIGURATION_CHANGE: 'MERCHANT_CONFIGURATION_CHANGE',
  CUSTOMER_BEHAVIOR_SHIFT: 'CUSTOMER_BEHAVIOR_SHIFT',
  TEMPORARY_NETWORK_FAILURE: 'TEMPORARY_NETWORK_FAILURE',
  UNKNOWN: 'UNKNOWN',
} as const;

export type CandidateHypothesis = typeof CANDIDATE_HYPOTHESES[keyof typeof CANDIDATE_HYPOTHESES];
...
export type RecoveryRecommendationAction = typeof RECOVERY_RECOMMENDATIONS[keyof typeof RECOVERY_RECOMMENDATIONS];

// ─── Evidence Collection Budget Caps ───────────────
export const EVIDENCE_BUDGET = {
  MAX_TOOL_CALLS: 10,
  MAX_INVESTIGATION_TIME_MS: 15_000,
  MAX_EVIDENCE_ITEMS: 25,
  MAX_RESULT_ROWS: 50,
  MAX_LLM_TOKENS: 2_000,
} as const;

// ─── Prompt Registry ───────────────────────────────
export const PROMPT_REGISTRY = {
  INCIDENT_INVESTIGATOR_V1: {
    promptId: 'incident-investigator',
    version: 'v1.0.0',
    purpose: 'Synthesize multi-dimensional financial evidence and score root cause hypotheses without hallucination',
  },
  HYPOTHESIS_EVALUATOR_V1: {
    promptId: 'hypothesis-evaluator',
    version: 'v1.0.0',
    purpose: 'Evaluate candidate hypotheses against observed metrics and conflicting signals',
  },
  RECOMMENDATION_ENGINE_V1: {
    promptId: 'recommendation-engine',
    version: 'v1.0.0',
    purpose: 'Propose structured recovery actions with risk and friction assessments',
  },
} as const;
